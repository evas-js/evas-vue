    ...
    if (data.$data) {
      logger.methodCall(`${this.entityName}.apiDataFetched`, arguments, () => {
        ...
        data.$data.forEach((sub) => {
          ...
          if (![undefined, null].includes(sub.totalRows))
            model.totalRows = sub.totalRows;
...
Model.fetch = function (name, args, cb) {
  ...
  return this.callApiRoute(name, args, (data, res) =>
    this.apiDataFetched(data, res, name, cb)
  );
};
...
Model.prototype.fetchRelation = function (name, args, cb) {
  return logger.methodCall(
    `${this.entityName}.fetchRelation`,
    arguments,
    () => {
      const { foreignModel, multiple } = this.relation(name);
      foreignModel[multiple ? "fetchList" : "fetchOne"](
        args,
        (data, entities, res) => {
